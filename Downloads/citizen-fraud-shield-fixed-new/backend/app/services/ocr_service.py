...
def _is_executable(path: str) -> bool:
    """True if `path` points to an existing, executable file."""
    return bool(path) and os.path.isfile(path) and os.access(path, os.X_OK)


def _check_tesseract_availability() -> bool:
    """
    Locate a working Tesseract binary and configure pytesseract to use it.

    Detection order (first match wins):
      1. shutil.which("tesseract")           -> resolved from PATH
      2. /usr/bin/tesseract                  -> standard Debian/Ubuntu apt location
      3. /usr/local/bin/tesseract             -> common manual/compiled-from-source location
      4. $TESSERACT_CMD                       -> explicit override via environment variable

    Once a candidate path is found, pytesseract.pytesseract.tesseract_cmd is set and
    the installation is verified with pytesseract.get_tesseract_version(). Verification
    failures of get_languages() elsewhere never cause this function to report the
    binary as missing — only the *absence* of an executable binary does that.
    """
    logger.info("Checking Tesseract availability. Current PATH=%s", os.environ.get("PATH", ""))
    ...
    candidates.append(("/usr/bin/tesseract", "/usr/bin/tesseract"))
    candidates.append(("/usr/local/bin/tesseract", "/usr/local/bin/tesseract"))

    env_path = os.environ.get("TESSERACT_CMD", "")
    if env_path:
        candidates.append(("TESSERACT_CMD env var", env_path))
    ...
    for source, path in candidates:
        if _is_executable(path):
            ...
            logger.info("Tesseract binary located via %s: %s", source, path)
            break
        else:
            logger.info("Tesseract not found via %s (checked path: %s)", source, path)

    if not resolved_path:
        logger.error(
            "No executable Tesseract binary found. Checked: shutil.which, /usr/bin/tesseract, "
            "/usr/local/bin/tesseract, TESSERACT_CMD env var. PATH=%s",
            os.environ.get("PATH", ""),
        )
        return False

    pytesseract.pytesseract.tesseract_cmd = resolved_path
    logger.info("Configured pytesseract.tesseract_cmd = %s (source: %s)", resolved_path, resolved_source)

    # Verify the binary actually runs.
    try:
        version = pytesseract.get_tesseract_version()
        logger.info("Tesseract binary verified. Version: %s", version)
    except Exception as exc:
        logger.error("Tesseract binary found at %s but failed to execute: %s", resolved_path, exc)
        return False
...
def _get_available_languages() -> List[str]:
    ...
    get_languages() is unreliable across some Tesseract builds/versions, so any
    failure here is logged and swallowed — it must never block OCR from running.
    """
    try:
        langs = pytesseract.get_languages(config="")
        logger.info("Tesseract reports installed languages: %s", langs)
        return langs
    except Exception as exc:
        logger.warning(
            "pytesseract.get_languages() failed (non-fatal, continuing with 'eng' assumption): %s",
            exc,
        )
        return ["eng"]
...
def _run_tesseract(image: Image.Image) -> Tuple[str, float, str, str]:
    ...
    logger.info("OCR run starting. tesseract_cmd=%s lang=%s", pytesseract.pytesseract.tesseract_cmd, lang_spec)

    try:
        raw_string_text = pytesseract.image_to_string(image, lang=lang_spec).strip()
        logger.info("OCR run finished successfully with lang='%s'", lang_spec)
        ...
        # Common cause: 'hin' traineddata not actually installed even though the
        # language string was requested. Fall back to English-only automatically.
        ...
        try:
            ...
            used_lang = "eng"
            logger.info("OCR fallback run finished successfully with lang='eng'")
        except (pytesseract.TesseractNotFoundError, FileNotFoundError) as inner_exc:
            logger.error("Tesseract binary is not installed or missing from PATH: %s", inner_exc)
            raise OCRProcessingError(
                "Tesseract OCR engine is not installed or available on the server."
            ) from inner_exc
...
def extract_text(file_bytes: bytes, filename: str) -> OCRResult:
    ...
    """
    logger.info("extract_text() called for filename=%s (%d bytes)", filename, len(file_bytes or b""))

    if not file_bytes:
        raise OCRProcessingError("The uploaded image is empty.")

    # 1. Verify Tesseract installation (only raises 422 if no executable binary exists)
    if not _check_tesseract_availability():
        logger.error("Tesseract OCR binary check failed for filename=%s.", filename)
        raise OCRProcessingError(
            "Tesseract OCR engine is not installed or available on the server."
        )
